...
/*
 * JDBC(Java DataBase Connectivity) 라이브러리를 이용하영 DB자로 처리하기
  	
  - 데이터베이서 처리순서
   1. 드라이버 로딩 ==> 라이브러리를 사용할 수 잇게 메모리로 읽어 들이는 작업
   		Class.forName("oracle.jdbc.driver.OracleDriver");
   		==> JDBC API 버전4 이상에서는 생략할 수 있다.
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3. 질의
				...
				//3-2 Statement 객체 또는 PreparedStatement객체 생성
				//	(connection 객체를 이용해서 생성한다.)	
					stmt = conn.createStatement();
				//3-3. SQL문 실행 (실행할 SQL문이 select문이기 때문에 결과가 ResultSet객체에 저장되어 반환된다.)
					rs = stmt.executeQuery(sql);
			
			// 4.결과 처리 ==> 한 레코드씩 화면에 출력하기
					
			//ResultSet에 저장된 데이터를 차례로 꺼내오려면 반복문과 next()메서드를 이용한다.
			//rs.next() ==> ResultSet객체의 데이터를 가리키는 포인터를 다음번째 레코드 위치로 이동시키고 그곳에 데이터가 있으면 true를 반환하고 없으면 false를 반환한다.
				while(rs.next()) {	
					// 포인터가 가리키는 곳의 데이터를 가져오기
					// 형식 1) rs.get자료형 이름 ("컬럼명");
					// 형식 2) rs.get자료형 이름 (컬럼번호); ==>컬럼번호는 1번부터 시작한다.
